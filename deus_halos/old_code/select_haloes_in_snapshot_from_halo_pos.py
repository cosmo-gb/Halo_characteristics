...
def read_binary_file(path_cube, name_file, halo_cube, cdm, Rvir, Rlim, N_marge, N_cubes, L_box):
    ...
    out_num, out_array, out_R_and_M = compute_R_delta(cdm_all[1],x_new,y_new,z_new)   
...
def check_if_file_exist(path_cube,name_file,halo_cube,N_cubes):
    N_cube_loop = np.int(2**halo_cube[-1][0]) # N_cube_loop is an integer between [1,8]
    file_existence = np.zeros(N_cube_loop,dtype=int) # 0 = the file does not exist
    for c in range(N_cube_loop): 
        my_cube = halo_cube[c+1]
        c_i = np.array((my_cube[0], my_cube[1], my_cube[2])) # c_x, c_y and c_z integers between [0,N_cube_1D-1]
        N_cube_1D = np.int(np.cbrt(N_cubes)) # cube numbers in 1 direction in the simulation box
        ...
        if c == 0 :
            num_main = num
        #print('num =',num)
        file_c_ex = isfile(path_cube+name_file+'_'+str('{:d}'.format(num).zfill(5)))
        if file_c_ex == True :
...
def beyond_FOF_func(cdm, Rvir, path_cube, name_file, f, name_halo_f, path_out, 
    ...
    if halo_final_pb != 0 :
        ...
        N_zeroes_saved = pd.DataFrame(np.array([name_halo_f,halo_final_pb]),columns=['halo name','number of zeroes in the final data'])
        N_zeroes_saved.to_csv(path_out+'Problem/zeroes_'+name_halo_f+'.dat',index=False)
    ...
    if N_cube_loop != ind_found_part.size:   
        ...
        cubes_pb_saved.to_csv(path_out+'Problem/cubes_'+name_halo_f+'.dat',index=False)
    if file_exist_pb == True :
        print(list([name_halo_f,num_main]))
        print('There is a file existence issue !')
        file_issue_saved = pd.DataFrame(list([num_main]),columns=['num main cube'])
        file_issue_saved.to_csv(path_out+'Problem/file_issue_'+name_halo_f+'.dat',index=False)
...
path_prop_halo_pos = '../../../../DEUSS_648Mpc_2048_particles/mass_bin_data/z_'+redshift_str+'/'+cosmo+'/halos_position/halo_properties/'
# path for load binary data
if cluster == True :
    path_cube = '/net/nas10c/data/gbonnet/DEUS_snapshot/z_'+redshift_str+'/'+cosmo+'/'
else :
    path_cube = '../../../../DEUSS_648Mpc_2048_particles/mass_bin_data/z_'+redshift_str+'/'+cosmo+'/snapshot/'  # path where I look for the binary data
# path for save the data
path_out = '../../../../DEUSS_648Mpc_2048_particles/mass_bin_data/z_'+redshift_str+'/'+cosmo+'/most_massive/'
##################################################################################
...
prop_num_save, R_and_M_save = np.zeros((N_haloes,17)), np.zeros((N_haloes,13))  # prop of haloes I am interested in
...
cdm = np.array((prop[:,2], prop[:,3], prop[:,4])) # halo center FOF (gravitational potential minimal point), so correct (Mpc/h)
cube_num, halo_num = np.array(prop[:,5],dtype=int), np.array(prop[:,6],dtype=int)
...
for f in indices_f[0:4]: # loop on haloes N_haloes
    ...
    halo_name_f = name_file+'_'+str('{:d}'.format(cube_num[f]).zfill(5))+'_'+str('{:d}'.format(halo_num[f]).zfill(5))
    ...
    if file_exist_pb == False : # halo I am interested in
        ...
        cdm_pot_x, cdm_pot_y, cdm_pot_z = cdm_all[1] #prop[f,6], prop[f,7], prop[f,8] # halo center (Mpc/h)
        cdm_fof_x, cdm_fof_y, cdm_fof_z = prop[f,2], prop[f,3], prop[f,4] # halo center (Mpc/h)
        cdm_dens_x, cdm_dens_y, cdm_dens_z = cdm_all[2] #prop[f,9], prop[f,10], prop[f,11] # halo center (Mpc/h)
