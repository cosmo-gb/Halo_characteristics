...
# -*- coding: utf-8 -*-
"""
Created on Sun Dec 26 15:27:12 2021
...
"""
This code looks for paticles of a halo knowing the halo position and size precisely 
(cdm_pot and Rvir) in the full simulation box and save it somewhere. 
The goal is to go beyond the Friend of Friend (FOF) algorithm
...
Its goal is to save the particles of the haloes.
...
if cluster == True : # case where I apply this code on the cluster
    # for the cluster array run at different mass:
    # 0,1,2 <=> lcdm, mass=12,13,14
    # 3,4,5 <=> rpcdm, mass=12,13,14
    # 6,7,8 <=> wcdm, mass=12,13,14
    # for the cluster array run at different redshift:
    # 0, 1, 2, 3, 4 <=> lcdm, redshift = 0 ; 0.5 ; 1 ; 1.5 ; 2
    # 5, 6, 7, 8, 9 <=> rpcdm, redshift = 0 ; 0.5 ; 1 ; 1.5 ; 2
    # 10, 11, 12, 13, 14 <=> wcdm, redshift = 0 ; 0.5 ; 1 ; 1.5 ; 2
    n_parallel = np.int(sys.argv[1]) # I import a number from a terminal command line
    num_cosmo_red = np.int(sys.argv[2])
    if num_cosmo_red < 5 :
        cosmo = 'lcdm'
    elif num_cosmo_red < 10 :
        cosmo = 'rpcdm'
    else :
        cosmo = 'wcdm'
    reste = num_cosmo_red % 5
    if reste == 0 :
        z_str = '0'
    elif reste == 1 :
        z_str = '0_5'
...
def read_binary_file(path_cube, name_file, halo_cube, cdm, Rvir, Rlim, N_marge, N_cubes, L_box):
    ...
    halo_final = np.array((x_new, y_new, z_new, vx_new*v_ren, vy_new*v_ren, vz_new*v_ren, ide))
    ind_sphere_delta = spherical_selection(x_new,y_new,z_new,Rvir,cdm)
...
def beyond_FOF_func(cdm, Rvir, path_cube, name_file, f, name_halo_f, N_vir_f, path_out, 
                    L_box=L_box,N_cubes=512,factor=5,marge=100):
    ...
    Rlim_f = Rlim[f]
    N_marge = np.int(N_vir_f) * marge
    if N_marge > 10**7 :
        print('Be carefull, N_marge is very large !!!')
    print(path_cube, name_file, my_cubes, cdm_f, Rvir[f], Rlim_f, N_marge, N_cubes, L_box)
    halo_final_all = read_binary_file(path_cube, name_file, my_cubes, cdm_f, Rvir[f], Rlim_f, N_marge, N_cubes, L_box)
    halo_final, halo_sphere_delta = halo_final_all[0], halo_final_all[1] # data and problem
    ...
    if save_data == True :
        ...
        #halo_final_csv.to_csv(path_out+'data/'+name_halo_f+'_'+str(factor)+'_Rvir.dat',index=False)
        file_save = open(path_out+'data/'+name_halo_f+'_'+str(factor)+'_Rvir.dat', 'w')
        file_save.write('# data of the DEUS halo '+name_halo_f+' and its surrounding (box of '+str(factor)+' Rvir (BN98)) \n')
        file_save.write('# L_box=648Mpc/h, '+cosmo+'w7, z = '+z_str+' '+mass_str+' haloes, (BN98) \n')
        file_save.write('# x (Mpc/h) in the box, y (Mpc/h), z (Mpc/h), v_x (km/s) in the box frame, v_y (km/s), v_z (km/s), particle identity \n')
        np.savetxt(file_save,halo_final)
        file_save.close()
        #halo_sphere_delta_csv = pd.DataFrame(halo_sphere_delta,columns=['x (Mpc/h)','y','z','v_x (km/s in box frame not in halo s)','v_y','v_z','ide'])
        #halo_sphere_delta_csv.to_csv(path_out+'data/'+name_halo_f+'_sphere_1_Rvir.dat',index=False)
    return()
...
path_prop = '../../../../DEUSS_648Mpc_2048_particles/mass_bin_data/z_'+z_str+'/'+cosmo+'/'+mass_str+'/halo_properties/'
# path for load binary data
if cluster == True :
    path_cube = '/net/nas10c/data/gbonnet/DEUS_snapshot/z_'+z_str+'/'+cosmo+'/'
else :
    path_cube = '../../../../DEUSS_648Mpc_2048_particles/mass_bin_data/z_'+z_str+'/'+cosmo+'/snapshot/'  # path where I look for the binary data
# path for save the data
path_out = '../../../../DEUSS_648Mpc_2048_particles/mass_bin_data/z_'+z_str+'/'+cosmo+'/'+mass_str+'/'
##################################################################################
prop_all = np.array(pd.read_csv(path_prop+'properties_sel.dat'))
N_haloes = len(prop_all)
print('N_haloes =',N_haloes)
names_all = prop_all[:,0]
R_vir_all = prop_all[:,1]
N_vir_all = prop_all[:,2]
cdm_all = np.array((prop_all[:,6], prop_all[:,7], prop_all[:,8])) # halo center FOF (gravitational potential minimal point), so correct (Mpc/h)
N_cores = 128
...
#dis_max = 50
#indices_f = np.where( (prop_all[:,6] < dis_max) & (prop_all[:,7] < dis_max) & (prop_all[:,8] < dis_max) )[0]
#print(indices_f)
for f in range(f_begin,f_end): # loop on haloes N_haloes
#for f in indices_f : # loop on haloes N_haloes
    print('f =',f)
    halo_name_f = names_all[f]
    print('name f =',halo_name_f)
    N_vir_f = N_vir_all[f]
    print('N_vir_f =',N_vir_f)
    print('R_vir_f =',R_vir_all[f])
    ###########################################################################
    # main computation
    print('cdm_f =',cdm_all[:,f])
    beyond_FOF_func(cdm_all, R_vir_all, path_cube, name_file, f, halo_name_f, N_vir_f, path_out) 
    print('ok')
    #break
t_fin = time.time()
