...
class halo_with_sub(smooth_halo,tidal_radius):
    ...
    def select_subhalos(self,dN_dm,m_min,m_max,N_sub_m_bin,m_sub,f_remove) :
        ...
        for b in range(N_sub_m_bin) :
            my_int = integrate.quad(lambda m: dN_dm(m), m_bin[b], m_bin[b+1])
            N_sub_bin[b] = self.around_integer_from_float(my_int[0]) # number of subhalos by subhalo mass bin
    ...
    def generate_halo_with_sub(self,kind_profile_main, kind_profile_sub, kind_profile_pos_sub,
                               ...
                               res=0.001,r_shell_binning='logarithmic'):
        ...
        main_halo = self.smooth_halo_creation(kind_profile_main,N_part=N_main,N_bin=N_bin_main,
                                              ...
                                              r_shell_binning=r_shell_binning,a_ax=a_ax_main,b_ax=b_ax_main,c_ax=c_ax_main)
        ...
        dN_dm = self.subhalo_mass_function(M_sub*fac_fake, m_min, m_max, delta) # subhalo mass function
        ...
        print('N_fake_subhalos =',N_sub_tot)
        ...
        print('N_fake_subhalos =',N_sub_fake)
        for s in range(N_sub_fake) : # loop on each individual subhalo
            ...
            ############################################################################################"
            # I add the particle of this subhalo s to the total halo
        ...
        N_tot = M_tot/m_part
        f_sub_fake = np.sum(N_part_sub_fin)/N_tot # subhalo mas fraction that I get
        f_remove = f_sub/f_sub_fake # fraction I need to get at the end
        ####################################################################### subhalo mass function
        dN_dm = self.subhalo_mass_function(M_sub, m_min, m_max, delta) # subhalo mass function
        ind_sub_use = self.select_subhalos(dN_dm,m_part,m_max,N_sub_m_bin,N_part_sub_fin*m_part,f_remove)
        N_part_sub_halos = N_part_sub_fin[ind_sub_use]
        f_sub_end = np.sum(N_part_sub_halos)/N_tot
        print('f_sub_final better =',f_sub_end)
        ###############################################################################################
        ...
        print('f_sub_initial =',f_sub_init)
        print('N_main =',N_main_get)
        print('N_part_sub_final better =',np.sum(N_part_sub_halos))
        print('N_tot =',len(halo_tot))
